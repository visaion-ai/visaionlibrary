"""
This config is for the HYHUNet-RegNetX-800mf-2X model.
...
checkpoint = "{{'$VISAION_CHECKPOINT:/root/data/visaion/visaionserver/.visaion/weights/regnetx-800mf_8xb128_in1k_20211213-222b0f11.pth'}}" # auto config
...
model = dict(
    ...
    decode_head=dict(
        ...
        input_channel_list=[32, 64, 128, 288, 672],
        last_scale_factor=2,
        conv_cfg=None,
        norm_cfg=dict(type="SyncBN", requires_grad=True),
    ...
    auxiliary_head=dict(
        type="STDCHead",
        _scope_="mmseg",
        in_channels=128,  # CAUTION: this value is from the backbone output channels, index is in_index of the outputs of the backbone
        channels=64,
        num_convs=1,
        num_classes=num_classes,
