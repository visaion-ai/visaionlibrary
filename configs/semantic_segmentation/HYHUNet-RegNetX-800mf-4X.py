...
model = dict(
    ...
    decode_head=dict(
        ...
        last_scale_factor=4,  # means the last upsample scale factor is 2, corresponding to the 4x in the model name
        conv_cfg=None,
        norm_cfg=dict(type="SyncBN", requires_grad=True),
...
# training config ----------------------------------------------------------------------------------------
_work_dir = (
    "/root/projects/visaionlib/workspace/debug/HYHUNet-RegNetX-800mf-4X"  # auto config
)
