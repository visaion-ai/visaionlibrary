...
def convert_coco_to_visaion(coco_json_path, image_dir, output_dir):
    ...
    for img_id in img_ids:
        ...
        # 保存标注文件
        json_path = os.path.join(output_dir, f"{os.path.basename(image_path).split('.')[0]}.json")
        with open(json_path, 'w') as f:
            json.dump(annotation_data, f)
        
        # 复制并重命名图片文件
        image_output_path = os.path.join(output_dir, f"{os.path.basename(image_path).split('.')[0]}.jpg")
        shutil.copy2(image_path, image_output_path)
        
        print(f"Processed: {image_file} -> {os.path.basename(image_path).split('.')[0]}")
